...
type Item = { id:number; date:string; amount:number; payment_method:string; category_id:number; notes?:string; attachment_path?:string };

export default function IncomePage() {
  const [cats, setCats] = useState<Cat[]>([]);
  const [items, setItems] = useState<Item[]>([]);
  const [form, setForm] = useState({ date:"", amount:"", payment_method:"Bank", category_id:"", notes:"" });
  const [fileById, setFileById] = useState<Record<number, File | null>>({});
  const [err, setErr] = useState<string | null>(null);

  async function load() {
    const [c, e] = await Promise.all([apiGet("/categories/income"), apiGet("/income")]);
    setCats(c); setItems(e);
  }
  ...
  async function create() {
    ...
    setForm({ date:"", amount:"", payment_method:"Bank", category_id:"", notes:"" });
    await load();
  }
  ...
  async function upload(id: number) {
    const f = fileById[id];
    if (!f) return;
    await apiUpload(`/income/${id}/attachment`, f);
    setFileById(prev => ({...prev, [id]: null}));
    await load();
  }
  ...
  return (
    <div className="card">
      ...
      <p className="small" style={{marginTop:12}}>
        Export: <a href={(process.env.NEXT_PUBLIC_API_BASE || "http://localhost:8000") + "/exports/income.csv"} target="_blank">income.csv</a>
      </p>
    </div>
  );
}
